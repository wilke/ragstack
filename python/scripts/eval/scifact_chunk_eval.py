...
async def evaluate_config(
    ...
    *,
    ...
    For each query we pull a ``retrieve_pool``-chunk hybrid candidate set, rerank the
    top ``rerank_pool`` of it with the cross-encoder, then build the ranked doc list
    as the reranked head followed by the retrieval-order tail, collapsed to unique
    ``doc_id``s (best rank first). Retrieving wider than the rerank pool is what lets
    ``recall@100`` actually reach 100 unique docs: a config emitting several chunks
    per doc collapses a 100-*chunk* pool to far fewer docs, which would structurally
    ...
    async def _eval_one(qid: str):
        ...
        async with sem:
            ...
            # Rerank only the top rerank_pool chunks; keep the rest in first-stage
            # order as the tail so recall@100 sees up to retrieve_pool unique docs.
            head, tail = pool[:rerank_pool], pool[rerank_pool:]
            reranked = await _rerank(query, [h.chunk for h in head])
            ranked_docs = _ranked_docs(
                [sc.chunk.doc_id for sc in reranked]
                + [h.chunk.doc_id for h in tail]
            )
            return {
...
def write_report(eval_stats, ingest_stats, keys, n_docs, n_q, source, live_eps, sig):
    ...
    struct = (
        "| config | size | overlap | #chunks | chunks/doc | overflow>cap | "
        "chunk s | ingest s |\n"
        "|" + "---|" * 8 + "\n" + struct_rows
    )
...
- **Retrieval:** `HybridRetriever` (Qdrant dense + ES BM25, RRF), tenant `public`,
  isolated `{SCIFACT_PREFIX}_<config>` stores. A wide chunk pool is retrieved per
  query; the top `rerank-pool` chunks are reranked by the crossencoder sidecar
  (`:50052`) and the remainder kept in first-stage order, so recall@100 sees up to
  `retrieve-pool` unique docs. Chunks are collapsed to unique doc_ids
...
def parse_args(argv=None):
    ...
    p.add_argument("--rerank-pool", type=int, default=100,
                   help="#top chunks reranked by the cross-encoder per query")
    p.add_argument("--retrieve-pool", type=int, default=300,
                   help="#chunks retrieved per query (>= rerank-pool); wider than "
