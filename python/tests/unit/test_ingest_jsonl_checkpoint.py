...
def test_checkpoint_none_filter(tmp_path):
    ...
    assert ingest_jsonl._read_checkpoint(ckpt) == {"line": 7, "doc_types": None}
...
def test_corrupt_checkpoint_falls_back_to_zero(tmp_path, garbage):
    ...
    assert ingest_jsonl._read_checkpoint(ckpt) == {"line": 0, "doc_types": None}
...
def test_legacy_bare_int_checkpoint_still_read(tmp_path):
    ...
    ckpt = tmp_path / "c.ckpt"
    ckpt.write_text("123")
    assert ingest_jsonl._read_checkpoint(ckpt) == {"line": 123, "doc_types": None}
