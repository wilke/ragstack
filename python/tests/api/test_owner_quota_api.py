...
async def test_admin_transfer_bypasses_the_quota_and_logs(client, caplog):
    await _own_directly("mine", "alice")
    await _own_directly("bob-1", "bob")
    await _own_directly("bob-2", "bob")  # bob is already at the limit

    # The bypass is logged from the router (it is the ACTOR's role that is
    # exempt here, not the recipient's — collections.py, not access.py), so
    # capture at the root logger rather than pin a module name that would
    # silently stop asserting anything if the log site ever moved.
