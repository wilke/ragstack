...
# verbatim). Secrets are generated once and read back thereafter.
#
...
# verbatim; a new tenant gets index = max(existing)+1. Base 24000 clears every
# occupied port on the deploy host (prod APIs 8000/8010, qdrant 6333/6343,
# vLLM 9001-9008, GoWe 8091, sidecars 50051-53, infra defaults).
...
if [[ -f "$MANIFEST" ]]; then
    while IFS=$'\t' read -r oname _oidx obase; do
...
stamp_file() {
    ...
    if [[ ! -f "$path" ]]; then
        ...
        warn "Re-run with --force to overwrite. Diff (existing vs generated):"
...
# 2. Manifest — append the row only if this is a new allocation (atomic).
...
# 3. Secrets — generated once, never rotated by a re-run.
...
# 4. Env file — the one operator-editable artifact: keep edits unless --force.
stamp_file "$ENV_FILE" "$(render_env)" keep tenant.env
chmod 600 "$ENV_FILE"

# 5. Start/stop wrappers — derived, deterministic; safe to regenerate.
stamp_file "$UP_SH" "$(render_up_sh)" overwrite up.sh
stamp_file "$DOWN_SH" "$(render_down_sh)" overwrite down.sh
chmod +x "$UP_SH" "$DOWN_SH"

# 6. Postgres database + role (only with --postgres) — checked before create.
if [[ -n "$PG_ADMIN_DSN" ]]; then
    ...
    if psql "$PG_ADMIN_DSN" -v ON_ERROR_STOP=1 -tAc \
        ...
        psql "$PG_ADMIN_DSN" -v ON_ERROR_STOP=1 \
            -c "CREATE ROLE \"$NAME\" LOGIN PASSWORD '$PG_PASSWORD'" >/dev/null
...
# 7. Preflight warnings (never fatal — provisioning is complete without them).
