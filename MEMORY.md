...
- **A feature wired only into `api/deps.py` does not reach a `gowe` tenant's ingest.** On `INGEST_BACKEND=gowe` the work is a CWL step in a container the engine launches — `pdf_extract.py -> ingest_shard.py` — so nothing on `app.state` and nothing in `tenant.env` reaches it. #596: DOI metadata enrichment existed, was settings-driven and had a pipeline hook, yet every upload-built collection was born bare, because `scripts/ingest_shard.py` built its `IngestionPipeline` without the enricher. Anything a gowe ingest must know has to travel as a workflow input (`documents.py::_gowe_inputs` → the CWL → the tool's argv), like the store URLs (#407) and the registry name (#563). **Check both backends when adding an ingest-time behaviour.**
- **The GoWe worker image is a second deployment artifact, and it is shared.** `/opt/ragstack/scripts/*` live inside `ragstack-worker.sif` (on `coconut`: `/scout/containers`, read by *every* worker group), so a new tool flag only exists once that image is rebuilt — and rebuilding it changes what all tenants run. An API that submits a flag an older image does not know makes argparse exit 2 and **every ingest task fail**; it does not degrade. Rebuild the image, then roll the APIs.
- **NCBI ID Converter: key on `requested-id`, not `doi`.** `https://pmc.ncbi.nlm.nih.gov/tools/idconv/api/v1/articles/` returns each record's `doi` in the *publisher's* case (`10.1128/JVI.02415-06`) while you ask in the normalized lowercase form, so matching on `doi` silently drops every uppercase-suffix publisher — most of ASM. `pmid` also comes back as a JSON **integer**, while `ingestion/jats.py` keeps pmid/pmcid as strings, so coerce it or the two corpora disagree on the field's type. And the widely-copied `ncbi.nlm.nih.gov/pmc/utils/idconv/v1.0/` URL is a 301 to the above — httpx does not follow redirects by default, so the old URL resolves nothing at all.
