...
**Index name is HAND-PINNED: `ragstack_lib_v1`.** Its build spec is identical to prod `ragstack_sfr_tok512`, so content-addressing would **collide**. Created with **both `max_segment_size` and `full_scan_threshold` pinned explicitly**, on its own Qdrant instance via `QDRANT_COLLECTION_ROUTES`. Pinning only the first is insufficient: at production's `full_scan_threshold=10000` KB and 16 KiB/vector the planner hands off from payload-index full scan to HNSW at ~625 vectors **per segment**. **Corrected (rev 3 arithmetic was wrong):** rev 3 put a 1000-PDF library at ~72,000 chunks over ~99 segments ≈ 730/segment — "on the boundary" — but 72 chunks/doc is the measured `fixed_tok256` figure (`reports/chunking-comparison-overview.md:222`), not `fixed_tok512`'s **36.2** (`:223`, and §-1). At 36.2 a 1000-PDF library is **~36,000 chunks**; over the same ~99-segment layout that is **~366/segment** — comfortably on the *full-scan* side, with the ~625 handoff not reached until **~1,700 documents**. Two caveats keep this a config decision rather than a settled one: the ~99-segment figure is itself carried over from the 24.8M-point prod collection (§16 Tier 2) and has never been measured on a library-sized index, and segment count moves with `max_segment_size` and merge timing. Which side of that handoff a library falls on must therefore still be a deliberate config decision, not a consequence of segment-merge timing. ~82 VMAs/library, ~800 per process; at 70% registration returns **503** and an operator provisions the next index (there is no automatic placement in v1).
