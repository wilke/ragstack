...
execution plane** that RAGStack submits ingest jobs to, co-located with the embedding fleet and
Qdrant, so a GoWe worker reaching them is a same-host call. The BV-BRC app is a
...
**What replaces G4 is a security requirement, not a gate.** RAGStack MUST verify token signatures
itself (offline, against a pinned issuer allowlist) and MUST NOT trust an identity assertion relayed
through any intermediary. BV-BRC tokens carry no audience claim, so a token is a plain bearer
credential; RAGStack therefore only accepts a credential presented directly by the client and only
ever forwards one downstream, never accepts one from a downstream service. §5.0 carries the
mechanism (offline verification, short cache TTL) and its honest bounds.

**The token's role in the GoWe / ingest path, stated correctly.** GoWe is a compute backend RAGStack
submits jobs to. It is **not** a credential provider and **not** an authentication authority for
RAGStack. The user token in that path has exactly one job — *delegated data access*: the worker
presents the user's own token to the BV-BRC Workspace / Shock to read that user's private files, so
the Workspace's ACL gates the read. GoWe carries the token; the Workspace verifies it. RAGStack
never asks GoWe "who is this user?", so GoWe's own submitter-authentication posture is a GoWe
deployment / network concern, not part of RAGStack's trust model. The real reason RAGStack verifies
tokens at all is **ownership-stamping and identity-gated reads** (§5.0, §6), not any property of an
upstream job-execution service.
...
keeps RAGStack's exposure to a compromised credential bounded rather than unbounded, and it is what
forbids the widening workaround §2.1 rejects.
...
in all cases. This is the invariant an upstream job-execution service that does not verify token
signatures violates; RAGStack MUST NOT inherit the habit.
...
#### Two verification boundaries — and what GoWe is not

RAGStack verifies the user's token at **two of its own boundaries**, using the same
`IdentityProvider` at both:

- **the query path** (chatbot → `/v1/query`) — to scope retrieval to what the caller may read;
- **the ingest *load* stage** — to authorize the write and **stamp ownership** (`tenant_id` /
  `library_id`) onto what is written (§6). Without verifying here, any submitter could claim any
  owner.

GoWe is a compute backend RAGStack submits jobs to — **not** a credential provider and **not** an
authentication authority for RAGStack. RAGStack never asks GoWe "who is this user?", so GoWe's own
submitter-authentication posture is a GoWe deployment / network concern, outside RAGStack's trust
model. The user token that travels the GoWe / ingest path has a **different, single job** —
delegated data access (§6): the worker presents the user's own token to the BV-BRC Workspace / Shock
so the Workspace ACL gates the read. GoWe carries that token; the Workspace verifies it.
...
- Format `un=…|tokenid=…|expiry=…|sig=…`. **The identity comes from the VERIFIED signature, never
  ...
  parser that reads `un` first and verifies later is exactly the failure this section forbids.
...
- **`expiry` absent MUST be treated as EXPIRED → 401.** A parser that treats an absent expiry as
  *not expired* is a common defect; RAGStack MUST invert that. Local expiry is checked **every
...
- Cache on `(issuer, token_id)`, TTL `min(300s, expires_at − now)`, bounded LRU. The cache key is
  used **only after** successful verification — otherwise it is attacker-chosen, and a bogus
  credential bearing a victim's `token_id` would poison the victim's entry.
...
#### RISK — bearer credentials (named, accepted, bounded)

**BV-BRC tokens carry no `aud` claim.** A token is therefore a plain bearer credential: nothing in
the credential says which service it was presented to, so possession is use. Offline verification
rejects a bad signature, but it does not turn a bearer credential into an audience-scoped one.
RAGStack bounds its exposure with the two rules below — **verify every credential itself, and never
act on a credential it did not receive directly from the client** — each with an honest limit:
...
```jsonc
{"doc_id":"3f2b…","library_id":"lib_…","tenant_id":"bvbrc:alice@bvbrc",
 "path":"/alice@bvbrc/efflux-pumps/smith2019.pdf","ws_object_id":"0A1A…",
...
**Worker survey.** Workers on the fleet carry `--workspace-stager`, so the Workspace stager
capability is *deployed*. **No worker carries `--shock-host`, so `shock://` is an unregistered scheme
in production today** — a config flip on the worker command line, **not new code**.

**Correction to the assumption this section was written under:** the `--workspace-stager` flag is
present on the general fleet but **not on the worker group a RAGStack ingest would target** (the
`--runtime none` group). So the Workspace stager is live on the fleet but **not on the group this
workflow runs in**. Treat that as a second config flip, and as a gate on §6 stage 0/6: **a RAGStack
ingest MUST NOT be scheduled to a group whose workers lack the `ws` scheme handler**, and stage 0
MUST fail fast with a legible error rather than producing an unresolvable `ws://` URI mid-run.
...
Because the worker never deletes its task directories, scratch accumulates without bound: each task
leaves a `task_<uuid>` tree (plus `..._tmp` / `..._output`). Worse, scratch left by a worker that has
since exited is **never reclaimed** — ownership died with the worker and there is no reaper.
...
after it:**
- A **cron reaper** over the GoWe worker task directories (`.../task_*`), deleting task trees older
  than a retention window (start at 72 h) **and** any subtree under a worker directory with no live
...
1. **The execution plane is already multi-worker.** GoWe runs several online workers; any library
   ingest stage runs on one of them, and which one is not knowable in advance.
...
1. **Firewall only, not architecture.** (a) Can BV-BRC's app service / chatbot reach the GoWe engine
   to submit jobs? GoWe uses BV-BRC token auth with anonymous disabled; the open part is the network
   path (NAT / VPN / allowlist), which is a GoWe deployment concern, not part of RAGStack's trust
   model (§5.0). (b) Can GoWe workers reach bv-brc.org's Workspace and Shock outbound? **Narrowed,
   not closed:** workers on the fleet run with `--workspace-stager` registered, which shows the
   capability is *deployed*, not that a Workspace call *succeeds*. One `WorkspaceLs` against a real
   root settles it.
2. **Chatbot server-to-server or browser?** Determines delegation vs CORS/CSRF; if browser, a bearer
   ...
   **Sharpened by §5.0:** BV-BRC tokens carry no `aud` claim, so a token leaked from a browser is a
   plain bearer credential usable anywhere it is accepted — another reason RAGStack must never accept
   a relayed credential and must keep the token out of logs.
...
5. **Will BV-BRC run an authorization server?** New in rev 4, and the only one that closes §5.0's
   bearer-credential gap (no `aud`, no introspection). Specifically: an OAuth 2.0 / OIDC issuer
   emitting audience-scoped, introspectable tokens. §5.0 tier 3 is designed to consume it with no
   change above `IdentityProvider`, so a "yes, eventually" costs nothing now — but a "no, ever" means
   the gap is permanent and should be recorded
...
9. §6 ingest workflow. **No longer gated** — GoWe is the execution plane.
   **9a. §6.2's cron reaper and the stage-0 free-space precondition ship WITH step 9, not after.**
   GoWe reclaims nothing and orphaned scratch already accumulates; adding 20 GB per run to an
