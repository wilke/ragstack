...
| [Synthesis stage](results/design/SPEC-synthesis-stage.md) | `PROPOSED` | The chunking study stops at evidence delivered into the context; this stage measures the answer — correct, faithful, cited precisely, abstains — and generation cost, per configuration and generator, on the pointed set where the gold answer is known by construction. Closes the two empty columns of the study report's decision ledger. |
...
| [Server-side prompt templates](prompt-templates.md) | `OPEN` | `/v1/query`'s generation prompt is a module constant (`llm.py:20`) with no config surface, while every adjacent knob — `llm_endpoint`, `llm_model`, `llm_max_context_chars` — is a `Setting`; no ADR records a choice to freeze it. Named server-side templates (caller picks an id and fills declared slots), **not** caller-supplied prompt text, which would make the API an open LLM proxy scoped by someone else's credential and would destroy the comparability an ablation arm needs. Bounds abuse and non-reproducibility, **not** prompt injection — a slot value is still caller text. Also settles the model-proxy question: the gate a proxy would sit behind already exists and is `/v1/query`. Raised by the literature demo (#544), which routes generation through a second service for want of this. |
...
| [Metadata consistency](metadata-consistency.md) | `PROPOSED` | One typed document-level schema (identity / bibliographic / structural, each field required, best-effort or source-specific — absent, never null) across OA, ASM and personal collections, and how the three existing regimes reach it. Measured 2026-09-15: ASM and personal chunks are the *same* producer (`enrich.index_metadata`), `section` is the boilerplate verdict (recoverable from `content`) while `section_title` is JATS-only and structurally unrecoverable for ASM; `spec_hash` covers only model/dim/chunk and no ASM row has an archive, so a payload-only update trips neither ADR-0002 nor a manifest. Dry-run → dev-tenant clone rehearsal with rehearsed ledger rollback → production canary, smallest corpus first; no snapshot repo exists on `:9200`. Names the numbers the running ASM audit must supply before anything is sized. Also reads the PubTator3 API docs out of their Angular bundle: the only NCBI raw-text annotator is the legacy `RESTful/request.cgi` — submit verified, retrieve not. |
| [User API keys](user-api-keys.md) | `PROPOSED` | Per-user API keys minted through the control plane and mapped to `bvbrc:` subjects (#584); six review blockers folded in; steps 0–2 landable independently |
