...
| **3** | Frontend: Grading view, pair view, adjudication, export — **done, #TBD** (`frontend/src/components/GradingView.tsx` + `components/grading/`, `lib/grading.ts`, typed calls in `api/client.ts`; the tab appears only for a caller with a batch, and the reader path renders `task.verdict` and never asks for `…/verdicts/{reader}`; screenshots in `frontend/docs/`) | readers use RAGStack, not the artifact | 2–3 days |
| **4** | Go: handlers to contract; conformance green on both — **done, #516** (`go/internal/grading/`: the CPython MT19937 reader-order port, pinned against fixtures generated by `/rag/envs/ragstack/bin/python3`, plus an in-memory store with append-only verdict versions and a conditional adjudication transition; `go/internal/api/handler_grading*.go`, all eleven operations; `go/internal/auth/` — the Go side had **no authentication at all**, and a surface whose subject is reader independence has none to enforce without a principal, so it reads the same `API_KEYS`/`API_KEY_TENANTS`/`API_KEY_ROLES` the Python server does and the keyless path is unchanged; `go/scripts/run_conformance_keyed.sh`. `conformance/test_grading.py` 12/12 against a keyed Go server; whole Go suite 170 passed / 71 skipped / 0 failed, from 148 / 83 / **10 failed**. No durable backend: the Go side has no DB layer to put one behind) | parity rule satisfied | 1–2 days |
