...
| span filter | #513 locator fix, dev **and** confirmation | `s0c_span_filter.py` | `artifacts/r31ext/` dev labels; `labels-conf-{scout,qwen}.jsonl` | `$BIG/work/r31ext-filtered/`, `labels-conf-*-filtered.jsonl`, `artifacts/conf-a/span-filter-{dev-verification,conf}.json` | none | seconds | ibid. §3, §4.5 | **done** — dev reliability 0.9205 → 0.9192, Δ = −0.0013 → PASS; conf applied 2026-09-14 (Scout 1,050 records filtered, Qwen 9) |
| conf-label | 30-reading confirmation labeling | `s0c_label.py`, `s0c_supervise.sh` | `pool/labeling_set.json` — **ids only**, 3,738 pairs | `$BIG/work/conf/labels/labels-conf-{scout,qwen}.jsonl` | `mango:8003` (Scout ×20 = 74,760 records) + `mango:8004` (Qwen ×10 = 37,380) | **Scout** 74,760/74,760 at **1.234 s/record** (≈ 25.6 h), done 2026-09-09. **Qwen** 37,380/37,380 at **6.04 s/record**, done 2026-09-11 02:13 UTC — it spanned the 09-10 reboot and resumed from checkpoint at 33,238 | ibid. §4.5 | **done** — 112,140 readings, 0 bad lines, 0 duplicate keys |
...
|---|---|---|---|---|
| ~~Qwen confirmation labeling~~ — **complete 2026-09-11** | 37,380 records (Scout's 74,760 done 09-09) | **6.04 s/record** at concurrency 5 on a server admitting 4 sequences; Scout **1.234 s/record** | took **≈ 62 h of wall clock** across both judges. Kept here because it is the rate another site would plan from | **Yes, and it would have been the biggest single win** — the record loop is embarrassingly parallel over `(topic, docno, presentation)` and idempotent per key, so four Qwen replicas would have finished in ≈ 9 h. Raising the server's `--max-num-seqs` is the precondition |
