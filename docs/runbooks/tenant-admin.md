...
with the *same build spec* as the origin — same model, dimension and chunking —
so the server mints the same physical store name the routes key on. A different
