...
| A4 | Job status is not readable across tenants | F·C | ✅ | #130; C leg closed by #628: `conformance/test_job_scope.py` under `make test-conformance-keyed`. Every keyed principal maps to its own subject, so P2 and B are both foreign to P1: P1 uploads, P1 and admin read the real status, P2 and B read `unknown` byte-identical to a missing id. Skips (absence, not credential) on an impl without `POST /v1/ingest/upload` — the Go scaffold today |
...
| # | Use case | Layer | State | Note |
|---|---|---|---|---|
| C1 | Upload a batch; job completes; chunks in both legs | C·L | ⚠️ | C leg partly closed by #630: `conformance/test_job_scope.py` (`make test-conformance-keyed`) has P1 upload `sample_small.txt` into a collection it owns and confirms the job's status is readable by P1/admin only, `unknown` to P2/B. Completion is now awaited and `failed` is refused (#644), so 'job completes' is proven on the C leg; chunks are still not read back from either leg by conformance, so 'chunks in both legs' remains unproven. Also missing: a multi-file upload, an upload into the `default` collection as a non-admin, and size/limit bounds — `test_upload_hardening.py` (#377) is F-only, not C |
