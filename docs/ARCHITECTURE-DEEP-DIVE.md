...
5. Token variant `_pack_spans_tokens` memoizes per-span token counts (`tok_of`), packs by the running per-span **sum** (which over-counts the joined chunk, so the emitted chunk is guaranteed `<=max_tokens`), and hard-splits a single over-budget sentence via `_token_split_span` (`chunkers.py:619-681`).
...
**Scalability & parallelization:** O(n) span detection + O(k) packing; token memo keeps tokenization O(k) per chunk not O(k²). No internal parallelism. Bottleneck: Punkt tokenization and (token path) per-span `count` calls. Overlap re-counting avoided via `span_tok` memo.
...
9. **Retry** — on a transient error (`_is_transient_error` walks `__cause__`/`__context__` for timeouts/5xx/pool-fail phrases) the batch retries up to `--batch-retries` with exponential backoff `_retry_delay` (1s,2s,4s… cap 30s); a 4xx/bad-input is non-transient and surfaces immediately (`ingest_jsonl.py:798-813`, `478-528`).
