...
| `MAX_UPLOAD_FILES`, `MAX_UPLOAD_BYTES_PER_REQUEST`, `UPLOAD_CONTENT_TYPES` | `POST /v1/ingest/upload` bounds (#202): at most **50** files per request, at most **500 MB** across them (the declared sizes are summed before anything is written, and the running total is enforced while streaming), and the content-type allowlist (default `application/pdf,text/plain,text/markdown,application/xml,text/xml`; a PDF must also start with `%PDF`) → `413` / `415`. Each file is still capped by `MAX_DOCUMENT_BYTES`, cut off at the byte while streaming. One ingest job per principal at a time (`429` + `Retry-After` while one is accepted/running; admins exempt) is not a setting. All three reported in `GET /v1/config` |
