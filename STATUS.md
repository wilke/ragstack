...
**Current tag:** [`v0.15.0`](https://github.com/wilke/ragstack/releases/tag/v0.15.0) at `28dbb8f` · **`main @ 3a95050`** (post-v0.15.0 merges below; **read [ROADMAP.md](ROADMAP.md) for what's next + what blocks what**)
...
**Unreleased (on `main`, post-v0.15.0) — decoupled embed-to-file + backpressured ingest ([#141](https://github.com/wilke/ragstack/issues/141)):** the near-term path to finish large ingests (e.g. `ragstack_sfr_semantic`) while Qdrant runs **capped** (`OPTIMIZER_CPU_BUDGET=12`) because `vm.max_map_count` can't be raised for ~7 days ([#140](https://github.com/wilke/ragstack/issues/140)). Under the cap, the coupled *embed→upsert* pipeline collapses on sustained upsert drops (`ResponseHandlingException`, #77 — killed 15/16 semantic shards at ~8.6%). The fix **splits vector generation from the DB write**: (1) a new `BatchSink` seam in `scripts/ingest_jsonl.py` (`ragstack/ingestion/sinks.py`: `QdrantSink` = unchanged coupled behavior; `FileSink` = sharded streaming gzip-JSONL + manifest) selected by **`--embed-out DIR`** — the GPU fleet embeds at full speed, never blocked by Qdrant; (2) a standalone **`scripts/qdrant_ingest_agent.py`** drains the shards as a *single, backpressure-gated writer* — it polls `QdrantVectorStore.collection_health()` (new: `status`/`optimizer`/`points`/`indexed`/`segments`) and throttles upserts while non-green / optimizing / over the unindexed-backlog or segment ceilings, keeping ≤`--max-inflight` batches in flight. Upsert-only + deterministic uuid5 ids ⇒ idempotent + resumable (per-shard checkpoint, `--resume`, `--delete-shards` to bound disk). Transient-retry helpers promoted to `ragstack/ingestion/retry.py` (shared). **Scope boundary:** backpressure fixes *upserts* only — the agent never touches `indexing_threshold` / never triggers a bulk rebuild (that deferred uncapped build is the #140 VMA crash). 18 new unit tests (FileSink round-trip/roll/manifest, QdrantSink parity, gate throttle + max-inflight, point-id parity, resume, dim-mismatch hard-fail); full suite 435 pass, ruff clean. **Live smoke (coconut, capped Qdrant):** 5-doc slice → 162×4096-d chunks embedded to a shard+manifest, drained into a throwaway collection green with points=162, **VMA delta +92** (well within the ~14k headroom), idempotent re-drain (points unchanged), collection dropped. **Feature only — the full semantic operator run is not yet done** (issue-acceptance step, operator-launched).
...
- [ ] Implement `GET /v1/documents` — needs a metadata store (Postgres) since the vector store only knows about chunks. Currently stub returns `[]`. **This is [#86](https://github.com/wilke/ragstack/issues/86) (doc registry), the single highest-leverage backend unblock per [ROADMAP.md](ROADMAP.md).** Gotcha from the live build: **CLI-built corpora bypass the API job registry**, so back the listing off the *served store* (Qdrant/ES), not the job table.
