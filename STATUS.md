...
**Deployed:** dev on [`v1.5.3`](https://github.com/wilke/ragstack/releases/tag/v1.5.3); asm-next, lucid-next, demo on [`v1.5.2`](https://github.com/wilke/ragstack/releases/tag/v1.5.2)
...
**Graph leg of the lifecycle ([#350](https://github.com/wilke/ragstack/issues/350), phase 6 of #201)**. `POST /v1/collections/{id}/graph[?version=n]` (owner-or-admin, 202, idempotent per version, one in-flight extraction per owner → 429) submits `cwl/graph-extract.cwl` **as the user** over one archived chunk version: `extract-graph` runs `LLMKGExtractor` over its `chunks.jsonl.gz` (#347 stamps, `concurrency` in flight) and emits a version-named delta (`manifest.json` rewritten `graph: true` + `triples.jsonl.gz` — the archive's `triples` role, `archive.write_triples`/`read_triples`) that GoWe post-stages onto `versions/<n>/` (the one intended overwrite); `load-graph` loads it scoped by `(tenant, collection)` within `graph_max_triples_per_collection` (200k; refused whole at exit 4 → job `graph_cap_exceeded`). Delivered completion records the version in the row's `graph_archived_versions` (additive column) — the flag #380's eviction half gates on. Restore replays the leg after the chunks. Off by default; never on the ingest path. Not yet: the eviction gate itself, the #122 graph arm, the default-on decision, the real-LLM throughput measurement.
...
├── repos/tenants/<t>/   # the four SERVING checkouts — dev at v1.5.3, the other three at v1.5.2
