...
**Current tag:** [`v0.15.0`](https://github.com/wilke/ragstack/releases/tag/v0.15.0) at `28dbb8f`
...
- [ ] **Ingest resume hardening** — [#71](https://github.com/wilke/ragstack/issues/71): bounded producer look-ahead, an optional `--batch-retries` in-process transient retry (the real lever for a **flapping endpoint** to converge — `done_ranges` stops the re-do-the-whole-file churn but a persistently-failing batch still exits non-zero), and #66 phase-2 `--chunk-concurrency`. **Prod action:** the lambda `next-batch` (tok256/tok512) build was churning on the pre-#70 code (checkpoint pinned at head, re-doing whole files under lambda flap). Fix: on the `/rag` checkout `git checkout main && git pull` (gets #68+#70), then restart the lambda job on `main` — **confirm it is not run with `--replace`** (which disables the `done_ranges` skip). The semantic (coconut) job is healthy and needs no restart.
