...
**`make test-conformance-keyed`** is the invocation that proves the authorization surface. It boots an in-memory API with four distinct keys mapped to four distinct subjects (admin / P1 / P2 / a second tenant), makes the registry pointer's target private so the **P2** persona is real, and runs everything. Expected as of this commit: **243 passed, 19 skipped**, then a second short-lived boot with `ALLOW_USER_COLLECTION_CREATE=false` for **2 passed** (matrix row A3), then a third boot with `INGEST_BACKEND=gowe` against a dead engine for **2 passed** (#609).

Read those counts as documentation, not as a gate — they drift as tests are added, and the matrix's *re-measure before quoting* rule applies. The **invariant** lives in the script: any skip tagged `RAGSTACK_CREDENTIAL_SKIP` fails the run, because on a server the script provisioned itself, "I had no credential for that" is a harness bug. The 19 legitimate skips are the identity suite (8, no OIDC provider configured), the create-gate file (2 — the first boot has creates enabled; it passes on the second), one duplicate-coverage skip, the un-triggerable store-503 body (2), the five `test_prompt_templates.py` tests that must name a template (none configured), and the one `test_chunk_method_capability.py` 422 branch that needs `INGEST_BACKEND=gowe` (it passes on the third boot). `test_grading.py` no longer skips: `/v1/grading/*` is mounted. The strict xfail that once lived here (#450, P2 could not list documents) was fixed by #422 PR-2 and is now a plain regression test.
...
- **Debugging a user-reported 5xx starts from the `Reference:` id on their screenshot** — that is `rid`, and [docs/runbooks/tracing-a-503.md](docs/runbooks/tracing-a-503.md) is the decision procedure from there: `grep rid=<id>` in `/rag/data/tenants/<tenant>/logs/api-<tenant>.log`, then read the summary line's stage fields to say *which leg* was slow. It also states what the timings cannot tell you, which matters before quoting `self_ms` at ADR-0006's Go trigger.
