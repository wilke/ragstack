...
launch() {                    # launch NAME cwd logfile -- cmd...   (detached, pid recorded)
  local name=$1 cwd=$2 log=$3; shift 3; [[ $1 == "--" ]] && shift
  if (( DRY )); then echo "  [dry-run] $name in $cwd:"; printf '      %q ' "$@"; echo "  >> $log"; return 0; fi
  say "  [$name] launching"
  ( cd "$cwd" && setsid nohup "$@" >> "$log" 2>&1 < /dev/null & echo $! > "$PIDS/$name.pid" )
}
...
if want stores; then
  ...
  start_instance qdrant "$IMG/qdrant.sif" \
    ...
    --bind "/rag/cache/load3corpus:/cache/load3corpus" \
  ...
  # neo4j-dev: password comes from dev's secrets.env (never printed). No /logs bind live.
  if ! instance_up neo4j-dev; then
    ...
    if [[ -n $NEOPW ]]; then
      start_instance neo4j-dev "$IMG/neo4j.sif" \
        --bind "$DATA/tenants/dev/neo4j/data:/data" --bind "$DATA/tenants/dev/neo4j/conf:/var/lib/neo4j/conf" \
  ...
  start_instance postgres "$IMG/postgres.sif" \
    --bind "$DATA/postgres/data:/var/lib/postgresql/data" --bind "$DATA/postgres/run:/var/run/postgresql" \
  ...
  start_instance neo4j "$IMG/neo4j.sif" \
    ...
    --env NEO4J_AUTH="neo4j/${NEO4J_PASSWORD:-ragstack}" -- || true

  wait_http http://127.0.0.1:6333/collections 120 "qdrant :6333" || fail=1
  wait_http http://127.0.0.1:6343/collections 120 "qdrant2 :6343" || fail=1
  wait_http http://127.0.0.1:24041/collections 120 "qdrant-dev :24041" || fail=1
  wait_http http://127.0.0.1:9200/ 180 "elasticsearch :9200" || fail=1
  wait_http http://127.0.0.1:24003/ 180 "elasticsearch-lucid :24003" || fail=1
  wait_http http://127.0.0.1:24043/ 180 "elasticsearch-dev :24043" || fail=1
  wait_http http://127.0.0.1:24046/ 120 "neo4j-dev :24046" || say "    (neo4j-dev: graph leg is disabled on every tenant; not fatal)"
  wait_http http://127.0.0.1:7474/ 60 "neo4j :7474" || say "    (neo4j: was already dead before the reboot; not fatal)"
...
if want sidecars; then
  ...
  wait_http http://127.0.0.1:50052/docs 300 "crossencoder :50052" || fail=1
  wait_http http://127.0.0.1:50053/docs 300 "embedding :50053" || fail=1
fi
...
if want sfr; then
  ...
  for i in 0 1 2 3 4 5; do wait_http "http://127.0.0.1:$((9001+i))/v1/models" 600 "sfr :$((9001+i))" || fail=1; done
fi
...
if want apis; then
  ...
  for spec in "lucid-next lucid 24000" "asm-next asm 24020" "dev dev 24040" "demo demo 24060"; do
    ...
    ( set -a; . "$tdir/config/tenant.env"; [[ -f $tdir/config/secrets.env ]] && . "$tdir/config/secrets.env"; set +a
      ...
      cd "$code" && setsid nohup /rag/envs/ragstack/bin/python -m uvicorn ragstack.api.main:app --host 0.0.0.0 --port "$port" \
        >> "$tdir/logs/api-$name.log" 2>&1 < /dev/null & echo $! > "$tdir/api-$name.pid"; cp "$tdir/api-$name.pid" "$PIDS/api-$name.pid" )
  done
  for spec in "lucid-next 24000" "asm-next 24020" "dev 24040" "demo 24060"; do set -- $spec; wait_http "http://127.0.0.1:$2/health" 180 "api $1 :$2" || fail=1; done
fi
...
if want uis; then
  ...
  for spec in "demo 5210" "lucid-next 5211" "asm-next 5212" "dev 8090"; do set -- $spec; wait_http "http://127.0.0.1:$2/" 120 "ui $1 :$2" || fail=1; done
fi
...
if want gowe; then
  ...
  if port_up 8091; then say "  [gowe-server] already listening — skipping"; else
    ...
    wait_http http://127.0.0.1:8091/api/v1/health 60 "gowe-server :8091" || fail=1
  fi
