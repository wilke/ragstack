...
func NginxTenants(f *registry.Fleet, cfg NginxConfig) ([]byte, error) {
	...
	// The JSON constants below sit inside nginx '…' literals, and nginx
	// interpolates $ inside a map value and inside a `return` body (verified).
	// They are safe only because every name reaching them passed checkValue,
	// which refuses [;{}$"'\ and newline] — so a hand edit is a
	// config-injection hole with no validator behind it. Say that IN the file:
	// the person about to edit it is not reading this source.
	b.WriteString("# DO NOT HAND-EDIT: the JSON literals below are safe only because the renderer refuses [;{}$\"'\\ and newline] in every name it writes — nginx interpolates $ inside map values and `return` bodies, so an edited name here is config injection.\n\n")
...
`, t.Name, cors, dist)
	...
	if f.Ctl.GatewayEnabled {
		...
		// The admin API block does NOT set `proxy_set_header Host` itself:
		// proxy-common.conf, included right above, already sets it, and a
		// second directive APPENDS a header rather than replacing the first.
		// The daemon was receiving two Host lines (verified), which is not a
		// harmless duplicate — Go's ValidHostHeader vets a single Host value,
		// and a request carrying two stops being something it can vet. One
		// Host header, from the include. If this block ever needs a different
		// Host, drop the include; do not stack a second directive on it.
		//
