#!/usr/bin/env bash
# Start the dedicated stores for tenant 'sandbox'.
# Generated by apptainer/new-tenant.sh — do not edit; re-run it to regenerate.
#
# Instances: qdrant-sandbox (http :24081, grpc :24082)
#            elasticsearch-sandbox (http :24083, transport :24084)
set -euo pipefail
IMG="/rag/apptainer/images"
TDIR="/rag/data/tenants/sandbox"
...
start qdrant-sandbox "$IMG/qdrant.sif" \
    --bind "$TDIR/qdrant/storage:/qdrant/storage" \
    --bind "$TDIR/qdrant/snapshots:/qdrant/snapshots" \
    --env QDRANT__SERVICE__HTTP_PORT=24081 \
    --env QDRANT__SERVICE__GRPC_PORT=24082 \
...
start elasticsearch-sandbox "$IMG/elasticsearch.sif" \
