#!/usr/bin/env bash
# Start the dedicated stores for tenant 'dev'.
# Generated by apptainer/new-tenant.sh — do not edit; re-run it to regenerate.
#
# Instances: qdrant-dev (http :24041, grpc :24042)
#            elasticsearch-dev (http :24043, transport :24044)
set -euo pipefail
IMG="/rag/apptainer/images"
TDIR="/rag/data/tenants/dev"
...
start qdrant-dev "$IMG/qdrant.sif" \
    --bind "$TDIR/qdrant/storage:/qdrant/storage" \
    --bind "$TDIR/qdrant/snapshots:/qdrant/snapshots" \
    --env QDRANT__SERVICE__HTTP_PORT=24041 \
    --env QDRANT__SERVICE__GRPC_PORT=24042 \
...
start elasticsearch-dev "$IMG/elasticsearch.sif" \
