...
func TestPostgresRefusesAnApptainerThatIsNotInstalled(t *testing.T) {
	...
	err := d.Postgres().Ready(context.Background(), spec)
	if !errors.Is(err, jobs.ErrRefused) || !strings.Contains(err.Error(), "not installed") {
		t.Fatalf("Ready without apptainer = %v, want a refusal that says so", err)
	}
}

// The runner's environment is sanitized: a tenant's secrets reach the ctl's
// process environment in some deployments, and a child that inherited them
// would put them in front of a container.
func TestRunArgvDoesNotForwardTheDaemonsEnvironment(t *testing.T) {
	dir := t.TempDir()
	bin := filepath.Join(dir, "env-dump")
	if err := os.WriteFile(bin, []byte("#!/bin/sh\nenv\n"), 0o755); err != nil {
		t.Fatal(err)
	}
	t.Setenv("TENANT_PG_PASSWORD", "hunter2")
	out, err := runArgv(context.Background(), []string{bin}, testTimeout)
	if err != nil {
		t.Fatal(err)
	}
	if strings.Contains(out, "hunter2") || strings.Contains(out, "TENANT_PG_PASSWORD") {
		t.Fatalf("the child inherited a secret from the daemon's environment: %s", out)
	}
	if !strings.Contains(out, "PATH=") {
		t.Errorf("the child got no PATH: %s", out)
	}
}

func TestRunArgvRefusesARelativeProgram(t *testing.T) {
	if _, err := runArgv(context.Background(), []string{"apptainer", "exec"}, testTimeout); !errors.Is(err, jobs.ErrRefused) {
		t.Fatalf("runArgv of a relative program = %v, want a refusal", err)
	}
}

func TestRunArgvStopsAProgramThatOverrunsItsDeadline(t *testing.T) {
	dir := t.TempDir()
	bin := filepath.Join(dir, "sleeper")
	// exec, so the process the runner kills IS the sleep: a forked child would
	// keep the output pipe open and the runner's WaitDelay — not its deadline —
	// would be what ends the call.
	if err := os.WriteFile(bin, []byte("#!/bin/sh\nexec sleep 30\n"), 0o755); err != nil {
		t.Fatal(err)
	}
	_, err := runArgv(context.Background(), []string{bin}, testShortTimeout)
	if err == nil || !strings.Contains(err.Error(), "timed out") {
		t.Fatalf("runArgv of a program that overruns = %v, want a timeout", err)
	}
}

func TestDetailTrimsCapturedOutput(t *testing.T) {
	if got := detail("   "); got != "" {
		t.Errorf("detail of whitespace = %q, want empty", got)
	}
	long := detail(strings.Repeat("x", 4096))
	if len(long) > errBodyBytes+8 {
		t.Errorf("detail is %d bytes; a step log is not a transcript", len(long))
	}
	if got := detail("could not\nconnect"); got != ": could not connect" {
